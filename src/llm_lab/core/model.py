...
from .transformer_block import ModelBlock
from ..config import TritonMindConfig
from ..utils.rope import precompute_freqs_cis


class TritonMindModel(nn.Module):
    """TritonMind 模型主干：embed → ModelBlock × N → RMSNorm，输出 hidden_states。"""
...
class TritonMindForCausalLM(nn.Module):
    """TritonMind 因果语言模型：TritonMindModel + lm_head，用于训练和生成。"""
    
    def __init__(self, config: TritonMindConfig):
        super().__init__()
        
        self.vocab_size = config.vocab_size
        self.model = TritonMindModel(config)
